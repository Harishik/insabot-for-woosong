...
Operational procedures for running, verifying, and releasing InsaBot for Woosong. For a quick start, see the [README](../README.md). For architecture, see [architecture.md](architecture.md).
...
```bash
cd insabot-for-woosong/backend
python -m venv .venv
...
The database service is named `db`. It uses `pgvector/pgvector:pg16`, exposes port `5432`, uses database `insabot`, username `insabot`, and stores data in the named volume `postgres_data`.
...
```bash
docker compose exec db psql -U insabot -d insabot -c "SELECT version();"
docker compose exec db psql -U insabot -d insabot -c "SELECT extname FROM pg_extension WHERE extname = 'vector';"
docker compose exec db psql -U insabot -d insabot -c "\dt"
docker compose exec db psql -U insabot -d insabot -c "\d document_chunks"
```

Verify the vector column and local corpus:

```bash
docker compose exec db psql -U insabot -d insabot -c "SELECT version_num FROM alembic_version;"
docker compose exec db psql -U insabot -d insabot -c "SELECT status, COUNT(*) FROM sourcedocument GROUP BY status;"
docker compose exec db psql -U insabot -d insabot -c "SELECT is_active, COUNT(*) FROM document_chunks GROUP BY is_active;"
docker compose exec db psql -U insabot -d insabot -c "SELECT embedding_vector <=> embedding_vector FROM document_chunks WHERE embedding_vector IS NOT NULL LIMIT 1;"
```

To run the host-side pgvector smoke check against Docker PostgreSQL:

```bash
$env:DATABASE_URL="postgresql+psycopg://insabot:insabot@localhost:5432/insabot"
...
For commercial use, obtain official permission from Woosong University before broad crawling or deployment.
