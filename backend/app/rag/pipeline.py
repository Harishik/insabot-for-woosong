...
def extractive_answer(chunks: list[RetrievedChunk], model_unavailable: bool = False) -> str:
    ...
    return (
        f"{prefix}"
        "Based on the available Woosong University sources, here is the verified guidance:\n\n"
        f"{source_lines}\n\n"
...
async def synthesize_from_sources(question: str, chunks: list[RetrievedChunk], language: str = "en") -> str:
    ...
    system = (
        "You answer Woosong University student questions using only the provided source excerpts. "
...
async def answer_question(session: Session, question: str, language: str | None = None) -> ChatResponse:
    ...
    if not enough_sources(chunks, sensitive):
        ...
        return ChatResponse(
            answer=FALLBACK_ANSWER,
            confidence="low",
            citations=[Citation(title=office.name, url=office.source_url or "https://english.wsu.ac.kr/main/index.jsp")],
