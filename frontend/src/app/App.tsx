...
import { WoosongLogo } from "@/components/brand/WoosongLogo";
        ...
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="Navigation menu">
          ...
          <div className="absolute right-0 top-0 flex h-full w-[82%] max-w-xs flex-col gap-4 overflow-y-auto bg-white p-4 shadow-soft">
            <div className="flex items-center justify-between">
              <WoosongLogo withWordmark size="sm" />
            ...
            <div className="space-y-2 border-t border-slate-100 pt-3">
              <p className="px-1 text-xs font-bold uppercase tracking-wide text-slate-400">{t("Woosong services")}</p>
...
function AppHeader({ title = "InsaBot", dark = false }: { title?: string; dark?: boolean }) {
  const { t } = useTranslation();
  return (
    <header className="flex items-center justify-between px-4 pb-3">
      <Link to="/" aria-label={t("Go to InsaBot home")} className="flex items-center gap-3 rounded-xl">
        <WoosongLogo badge size="sm" />
        <span>
          <span className={cn("block text-xs font-semibold", dark ? "text-slate-400" : "text-slate-500")}>{t("WSU AI Assistant")}</span>
          <span className="block text-lg font-bold">{title === "InsaBot" ? title : t(title)}</span>
        </span>
      </Link>
      <AppMenu dark={dark} />
    </header>
  ...
  return (
    <aside className="sticky top-0 hidden h-dvh w-72 shrink-0 flex-col gap-4 overflow-y-auto border-r border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 lg:flex">
      <Link to="/" className="flex items-center gap-3">
        <WoosongLogo badge size="md" />
        <span className="leading-tight">
          <span className="block text-lg font-extrabold text-brand-700 dark:text-white">InsaBot</span>
          <span className="block text-xs font-semibold text-slate-500 dark:text-slate-400">{t("WSU AI Assistant")}</span>
        </span>
      </Link>
      <nav className="space-y-1">
        {primaryNav.map((item) => (
...
function getSavedAnswers(): RenderedAnswer[] {
  ...
  try {
    return JSON.parse(window.localStorage.getItem("insabot_saved_answers") ?? "[]") as RenderedAnswer[];
  } catch {
    return [];
  }
}
...
function saveAnswer(answer: RenderedAnswer) {
  ...
  window.localStorage.setItem("insabot_saved_answers", JSON.stringify([{ ...answer }, ...saved].slice(0, 20)));
}
...
function ChatInput({ dark = false, onSend, disabled = false, placeholder = "Ask follow-up question..." }: { dark?: boolean; onSend?: (message: string) => void; disabled?: boolean; placeholder?: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className={cn("sticky bottom-0 mt-auto flex gap-2 border-t p-3 pb-[calc(0.75rem+var(--safe-bottom))]", dark ? "border-slate-800 bg-slate-950" : "border-slate-200 bg-white")}>
      <label htmlFor={questionId} className="sr-only">Ask InsaBot</label>
...
export function BrandPanel() {
  return (
    <aside className="hidden w-[300px] shrink-0 rounded-2xl bg-white p-6 shadow-soft lg:block">
      <h1 className="text-4xl font-extrabold leading-none text-brand-700">InsaBot<br />for Woosong</h1>
      <p className="mt-3 inline-flex rounded-lg bg-brand-600 px-3 py-2 text-lg font-bold text-white">WSU AI Assistant</p>
      ...
      <Panel className="mt-6 bg-brand-50">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-700">Helping students, faculty, and staff succeed at Woosong through smart, trustworthy AI assistance.</p>
      </Panel>
      ...
      <div className="mt-6 flex items-center gap-3 rounded-xl border border-slate-200 p-3">
        <WoosongLogo size="md" />
        <span className="text-sm font-bold text-slate-700">Woosong University<br />Smart Campus Integrated</span>
      </div>
    </aside>
  );
}
...
export function HomeScreen() {
  ...
  return (
    <PhoneFrame>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">{t("Welcome to Woosong, how can we help you today?")}</p>
            </div>
          </div>
      ...
      <section className="px-4">
        <Card className="overflow-hidden bg-gradient-to-br from-brand-600 to-brand-900 p-5 text-white">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-semibold text-blue-100">{t("WSU AI Assistant")}</p>
              <h2 className="mt-1 text-2xl font-extrabold">InsaBot</h2>
      ...
      <p className="mt-5 px-4 text-center text-xs text-slate-500">{t("Verified answers from official Woosong sources.")}</p>
      <BottomNav />
    </PhoneFrame>
  );
}
    ...
    <PhoneFrame>
      ...
      <section className="space-y-4 px-4">
        <Card className="bg-gradient-to-br from-brand-600 to-brand-900 p-5 text-white">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm text-blue-100">{t("WSU AI Assistant")}</p>
              <h2 className="mt-2 text-xl font-extrabold">{t("Hi, I’m InsaBot. How can I help you today?")}</h2>
            </div>
            <RobotMascot />
          </div>
        </Card>
        ...
        <form onSubmit={handleSubmit} className="rounded-xl border border-slate-200 bg-white p-2">
          <div className="flex gap-2">
            <label htmlFor="chat-landing-question" className="sr-only">Ask InsaBot</label>
        ...
        <p className="text-xs text-slate-500">{t("Verified answers from official Woosong sources. Sensitive topics are routed to human offices when needed.")}</p>
      </section>
      <BottomNav />
    </PhoneFrame>
  );
...
export function ChatConversation({ variant = "attendance" }: { variant?: "attendance" | "grades" }) {
  ...
  async function askInsaBot(message: string, nextLanguage: string | null = requestLanguage) {
    setQuestion(message);
    setAnswer(null);
    setError("");
    setIsLoading(true);
    try {
      ...
      setError(err instanceof Error ? err.message : "InsaBot could not reach the answer service.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  useEffect(() => {
    void askInsaBot(initialQuestion, requestLanguage);
  ...
  return (
    <PhoneFrame wide>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto px-4">
        ...
        {isLoading ? (
          <Panel className="space-y-3">
            ...
            <p className="text-sm font-semibold text-slate-500">{t("Checking official Woosong sources...")}</p>
          </Panel>
        ) : error ? (
          <Panel className="space-y-3 border-red-200 bg-red-50">
            ...
            {detectedService ? (
              <p className="text-sm text-red-700">{t("You can still use the official Woosong link above while the answer service is offline.")}</p>
            ) : null}
            <Button variant="outline" className="bg-white" onClick={() => void askInsaBot(question)}>
              {t("Try Again")}
            </Button>
          </Panel>
        ) : answer ? (
      ...
      <ChatInput onSend={(message) => void askInsaBot(message)} disabled={isLoading} />
    </PhoneFrame>
  );
}
      ...
      <div className="space-y-4 px-4">
        ...
        <Panel className="space-y-4 border-orange-200 bg-orange-50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-bold text-slate-900">{t("I could not verify this from the available Woosong University sources.")}</h2>
          </div>
...
export function HistoryScreen() {
  ...
  function clearHistory() {
    if (tab === "saved") {
      window.localStorage.removeItem("insabot_saved_answers");
      setSaved([]);
