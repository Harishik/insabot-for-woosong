...
import logoUrl from "@/assets/Woosong-University-Logo.png";
...
export type WoosongLogoProps = {
  /** Show the "Woosong University" wordmark next to the mark. */
    ...
    <span className={cn("inline-flex items-center gap-2", className)}>
      {badge ? (
        <span className={cn("grid shrink-0 place-items-center rounded-xl bg-white p-1 shadow-sm", markSizes[size])}>
          <img src={logoUrl} alt="Woosong University logo" className="h-full w-full object-contain" loading="eager" decoding="async" />
        </span>
      ) : (
        mark
      )}
