...
describe("api client", () => {
  ...
  it("starts crawls and creates handoff tickets with expected payloads", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse({ id: 3, seed_url: "https://english.wsu.ac.kr", status: "queued", page_limit: 20, created_at: "2026-05-25T00:00:00" }))
    ...
    await api.admin.startCrawl("https://english.wsu.ac.kr", 20);
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(1, "http://localhost:8000/api/admin/crawl", expect.objectContaining({
      method: "POST",
      body: JSON.stringify({ seed_url: "https://english.wsu.ac.kr", page_limit: 20 })
    }));
