...
def ollama_diagnostics(gate: Gate, require_live: bool) -> bool:
    ...
    try:
        embed_response = httpx.post(
            ...
            json={"model": embed_model, "input": "Woosong University release gate embedding check."},
            timeout=60,
        )
        if embed_response.status_code == 404:
            embed_response = httpx.post(
                f"{base_url}/api/embeddings",
                json={"model": embed_model, "prompt": "Woosong University release gate embedding check."},
                timeout=60,
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Release gate for InsaBot RAG readiness.")
    parser.add_argument(
        "--mode",
